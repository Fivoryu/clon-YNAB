...
type CommandResult = { id: string; version: number; amountMinor?: number; released?: boolean };
    type HistoryItem = { transactionId: string; kind: 'INCOME' | 'SPENDING'; date: string; amountMinor: number; category?: Category | null; state: 'ELIGIBLE' | 'PROTECTED' };
...
export default function Home() {
  ...
  const signOut = async () => { try { await call('/api/v1/auth/sign-out', { method: 'POST' }); setBudget(null); setSummary(null); setHistory(null); setMessage('Signed out.'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to sign out'); } };
  ...
  const loadHistory = async (targetMonth = historyMonth) => {
    if (!budget) return;
    try {
      const result = await call<HistoryResponse>(`/api/v1/budgets/${budget.id}/transactions${targetMonth ? `?month=${encodeURIComponent(targetMonth)}` : ''}`);
