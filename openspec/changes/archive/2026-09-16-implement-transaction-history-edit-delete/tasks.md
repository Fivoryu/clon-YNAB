...
- [x] 4.1 Complete `apps/api/test/transaction-history-prisma.test.ts` coverage for migration/backfill, restart durability, replacement chains, tombstone exclusion, active/archived category rules, audit actor identity, and raw-history rebuild equivalence; verify PostgreSQL is authoritative and the in-memory store is not used as durability evidence. <!-- sdd-owner: implementation -->
- [x] 4.2 Add two-client Prisma concurrency tests for budget locking, stale version rejection, concurrent edits, and safe idempotent replay; verify exactly one authorized mutation commits and the newer state is preserved. <!-- sdd-owner: implementation -->
